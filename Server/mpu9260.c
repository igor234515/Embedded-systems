...
void AK8963_Read_Magnet(I2C_HandleTypeDef *I2Cx, MPU9250_t *DataStruct) //MAGNETOMETER READ
{
	...
	if(readData & 0x01)// wait for magnetometer data ready bit to be set
	{
		...
		HAL_I2C_Mem_Read(I2Cx, AK8963_ADDRESS, AK8963_XOUT_L, 1, &rawData[0], 7, i2c_timeout);
		uint8_t c = rawData[6]; // End data read by reading ST2 register
	    if(!(c & 0x08))
	    { // Check if magnetic sensor overflow set, if not then report data
	    DataStruct -> magnetX = (int16_t)(((int16_t)rawData[1] << 8) | rawData[0]);
	    DataStruct -> magnetY = (int16_t)(((int16_t)rawData[3] << 8) | rawData[2]);
