...
{
    uint8_t checkM = 0;
    uint8_t iter_addr = AK8963_WHO_AM_I;
    while(checkM == 0)
    {
    	HAL_I2C_Mem_Read(I2Cx, AK8963_ADDRESS, AK8963_WHO_AM_I, 1, &checkM, 1, i2c_timeout);
    	iter_addr+=1;
    }
    uint8_t writeData = 0x22;
    HAL_I2C_Mem_Write(I2Cx, MPU9250_ADDR, INT_PIN_CFG, 1, &writeData, 1, i2c_timeout);
...
void AK8963_Read_Magnet(I2C_HandleTypeDef *I2Cx, MPU9250_t *DataStruct) //MAGNETOMETER READ
{
	...
	{ // wait for magnetometer data ready bit to be set
		HAL_I2C_Mem_Read(I2Cx, AK8963_ADDRESS, AK8963_XOUT_L, 1, &rawData[0], 7, i2c_timeout);  // Read the six raw data and ST2 registers sequentially into data array
		uint8_t c = rawData[6]; // End data read by reading ST2 register
	    if(!(c & 0x08))
	    { // Check if magnetic sensor overflow set, if not then report data
	    DataStruct -> magnetX = (int16_t)(((int16_t)rawData[1] << 8) | rawData[0]);  // Turn the MSB and LSB into a signed 16-bit value
	    DataStruct -> magnetY = (int16_t)(((int16_t)rawData[3] << 8) | rawData[2]);  // Data stored as little Endian
	    DataStruct -> magnetZ = (int16_t)(((int16_t)rawData[5] << 8) | rawData[4]);
	    }
	}
}
...
void MPU9250_Read_Temp(I2C_HandleTypeDef *I2Cx, MPU9250_t *DataStruct)
{
    ...
    HAL_I2C_Mem_Read(I2Cx, MPU9250_ADDR, TEMP_OUT_H_REG, 1, Rec_Data, 2, i2c_timeout);
...
void MPU9250_Read_All(I2C_HandleTypeDef *I2Cx, MPU9250_t *DataStruct)
{
    ...
    HAL_I2C_Mem_Read(I2Cx, MPU9250_ADDR, ACCEL_XOUT_H_REG, 1, Rec_Data, 14, i2c_timeout);
